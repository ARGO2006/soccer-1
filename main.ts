...
let secret_goal = sprites.create(img`
    ...
    3 3 3 3 3 3 3 3 3 3 3 3 3 3 3 3 
    `, SpriteKind.Secret)
tiles.placeOnRandomTile(secret_goal, assets.tile`myTile25`)
...
tiles.placeOnRandomTile(second_secret_goal, assets.tile`myTile26`)
...
for (let value of tiles.getTilesByType(assets.tile`myTile21`)) {
    ...
    tiles.placeOnTile(soccerPlayer, value)
    ...
    sprites.setDataNumber(soccerPlayer, "goalX", tiles.locationXY(tiles.getTilesByType(assets.tile`myTile24`)[0], tiles.XY.x))
    sprites.setDataNumber(soccerPlayer, "goalY", tiles.locationXY(tiles.getTilesByType(assets.tile`myTile24`)[0], tiles.XY.y))
...
game.onUpdateInterval(500, function () {
    for (let value of sprites.allOfKind(SpriteKind.FrogTeam)) {
        if (spriteutils.getSpritesWithin(SpriteKind.Ball, 20, value).length != 0) {
            value.follow(soccer_ball, 20)
        } else {
            value.follow(null)
            timer.after(1000, function () {
                story.spriteMoveToLocation(value, sprites.readDataNumber(value, "homeX"), sprites.readDataNumber(value, "homeY"), 20)
            })
        }
    }
    for (let value of sprites.allOfKind(SpriteKind.HatTeam)) {
        if (spriteutils.getSpritesWithin(SpriteKind.Ball, 20, value).length != 0) {
            value.follow(soccer_ball, 20)
        } else {
            value.follow(null)
